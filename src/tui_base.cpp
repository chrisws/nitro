...
// TuiBase::redraw
//
void TuiBase::redraw_header() const {
...
void TuiBase::redraw_input() const {
...
void TuiBase::set_thinking(bool on) {
  thinking = on;
...
void TuiBase::update_usage(int tokens_sec, const LlamaMemoryInfo &mem) {
...
//
// TuiBase content helpers
//
void TuiBase::append_line(const std::string &line) {
  std::lock_guard<std::mutex> lk(lines_mutex);
...
void TuiBase::append_token(const std::string &token) {
  token_acc += token;
  for (;;) {
...
//
void TuiBase::show_modal_popup(const std::string &message) {
  ...
  opts.y = py; opts.x = px;
  opts.rows = (unsigned)popup_h; opts.cols = (unsigned)popup_w;
...
void TuiBase::show_help() {
  append_line(ICON_SYS + "Commands:");
  append_line(ICON_SYS + "  /set    - Adjust generation parameters");
  append_line(ICON_SYS + "  /rl     - List recent files");
  append_line(ICON_SYS + "  /exit   - Exit Nitro");
  append_line(ICON_SYS + "  Ctrl+C  - Cancel current input");
}

void TuiBase::dismiss_modal_popup() {
  if (modal_plane) {
...
// Returns the chosen path, or "" on cancel.
//
std::string TuiBase::file_picker(const std::string &start_dir,
...
//
// ─── TuiBase::confirm_dialog ─────────────────────────────────────────────
//
bool TuiBase::confirm_dialog(const std::string &prompt) const {
  ncplane_erase(inputpl);
...
//
std::string TuiBase::readline_blocking() {
  input_buf.clear();
  cursor_pos = 0;
  history.reset_nav();
  redraw_input();
